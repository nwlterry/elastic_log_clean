...
import (
	...
	"github.com/nwlterry/elastic_log_clean/pkg/clean"
...
func run(args []string) int {
	fs := flag.NewFlagSet("elastic_log_clean", flag.ContinueOnError)
	...
	fmt.Fprintf(os.Stderr, "[elastic_log_clean %s] starting\n", Version)
	...
	tmp, err := os.MkdirTemp("", "elastic_log_clean_*")
	if err != nil {
		fmt.Fprintf(os.Stderr, "error: %v\n", err)
		return 2
	}
	...
	default:
		...
		if c.ShouldOmit(rel) {
			...
			cleaned := clean.ProcessBytes(c, data, rel)
	...
	b, err := yaml.Marshal(report{
		Tool: "elastic_log_clean", Version: Version, Input: inp, Output: out,
