#!/usr/bin/env python3
"""elastic_log_clean — obfuscate sensitive data in Elastic diagnostics and logs.

Modeled on OpenShift must-gather-clean:
  omit rules drop files; obfuscate rules rewrite contents/paths with
  consistent or static replacements and emit report.yaml.

Targets Elastic Support Diagnostics zips, extracted diagnostic trees,
cluster/node logs, Kibana/Fleet/APM logs, elasticsearch.yml dumps,
and stdin pipes.
"""
...
SECRET_PATTERNS = [
    ...
    ("api_key_header", re.compile(r"(?i)(api[_-]?key\s*[:=]\s*)([A-Za-z0-9+/=_\-]{8,})"), r"\1x-redacted-apikey-x"),
    ("elastic_api_key", re.compile(r"(?i)(ApiKey\s+)([A-Za-z0-9+/=_\-]{8,})"), r"\1x-redacted-apikey-x"),
    ("password_assignment", re.compile(r"(?i)((?:password|passwd|secret|token|bind_password|service_token|keystore\.seed|truststore\.password|keystore\.password|xpack\.security\.http\.ssl\.keystore\.secure_password|xpack\.security\.transport\.ssl\.keystore\.secure_password|cloud\.auth|bootstrap\.password)\s*[=:]\s*)([^\s,\"'}]+)"), r"\1x-redacted-secret-x"),
    ("quoted_password", re.compile(r'(?i)("(?:password|passwd|secret|token|api_key|authorization|bind_password)"\s*:\s*")([^"]+)"'), r'\1x-redacted-secret-x"'),
    ("basic_userinfo", re.compile(r"(?i)((?:basic\.auth\.user\.info|cloud\.auth)\s*[:=]\s*)(\S+)"), r"\1x-redacted-userinfo-x"),
...
def is_probably_text(path: Path, sample: bytes) -> bool:
    ...
    if suffix in SKIP_COPY_AS_BINARY:
        return False
    if suffix in TEXT_EXTENSIONS or path.name.lower() in {"elasticsearch.yml", "kibana.yml", "log4j2.properties", "jvm.options"}:
        return True
...
class MappingStore:
    ...
    def report(self) -> list[dict[str, Any]]:
        out = []
        for rec in sorted(self.replacements.values(), key=lambda r: r.replaced_with):
            out.append({"canonical": rec.canonical, "replacedWith": rec.replaced_with, "occurrences": [{"original": k, "count": v} for k, v in sorted(rec.occurrences.items())]})
        return out
...
def process_tree(cleaner: Cleaner, src: Path, dst: Path, workers: int) -> None:
    ...
    if workers <= 1:
        for path in files:
            ...
            (cleaner.omitted.append(rel) if status == "omit" else setattr(cleaner, "processed_files", cleaner.processed_files + 1))
        return
...
def default_config() -> dict[str, Any]:
    return {
        "omit": [
            ...
            {"type": "File", "pattern": "**/*private*key*"},
            {"type": "File", "pattern": "**/elasticsearch.keystore"},
            {"type": "File", "pattern": "**/users"},
            {"type": "File", "pattern": "**/users_roles"},
        ],
        "obfuscate": [
...
def build_parser() -> argparse.ArgumentParser:
    p = argparse.ArgumentParser(prog="elastic_log_clean", description="Obfuscate sensitive data in Elastic diagnostics, cluster logs, and server logs.")
    p.add_argument("-c", "--config", help="YAML config (omit + obfuscate). Default built-in Elastic rules.")
    p.add_argument("-i", "--input", help="Input file, directory, or .zip diagnostic archive.")
    p.add_argument("-o", "--output", help="Output directory or .zip path.")
    p.add_argument("-r", "--report", default="report.yaml", help="Replacement report path (do not share).")
    p.add_argument("-w", "--workers", type=int, default=os.cpu_count() or 4)
    p.add_argument("--stdin", action="store_true", help="Read text from stdin and write cleaned text to stdout.")
...
def main(argv: list[str] | None = None) -> int:
    ...
    with tempfile.TemporaryDirectory(prefix="elastic_log_clean_") as tmp:
        tmp_path = Path(tmp)
        ...
        if inp.suffix.lower() == ".zip":
            ...
            if cleaner.should_omit(rel):
                ...
                cleaner.processed_files += 1
    dump_yaml({"tool": "elastic_log_clean", "version": VERSION, "input": str(inp), "output": str(out), "processedFiles": cleaner.processed_files, "omittedFiles": sorted(cleaner.omitted), "replacements": cleaner.store.report(), "config": config, "warning": "Do not share this report. It maps original values to replacements."}, report_path)
    print(f"cleaned: {out}")
